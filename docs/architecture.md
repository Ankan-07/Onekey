...
Onekey is a two-tier application: a **FastAPI gateway** that proxies and
...
flowchart TB
    ...
    SDK -->|Bearer ok-...| V1
    Codex -->|Bearer ok-...| Responses
    Claude -->|x-api-key ok-...| Anthropic
    Browser --> Auth
    Auth --> SupaAuth
    Browser --> APIClient
    APIClient -->|JWT| Mgmt
...
sequenceDiagram
    ...
    C->>G: POST /v1/chat/completions (ok- key)
...
- Onekey key auth (`ok-...`) for `/v1/*` — Bearer header or `x-api-key`
...
- SHA-256 hashing for Onekey token lookup
...
| `KeychainKey` | `ok-` inference keys (hashed) |
