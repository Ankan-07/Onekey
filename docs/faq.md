...
Detailed frequently asked questions for Onekey.

## General

### How is Onekey different from OpenRouter?

Onekey is **self-hosted** and uses **your own** upstream provider keys.
...
### What happens when I request `onekey-high`?
...
`onekey-medium` or `/v1/messages` with `claude-sonnet-4-6`. All upstream
...
Yes. A `onekey-high` request that exhausts high-tier candidates continues
...
### Are Onekey keys hashed?

Yes. Only SHA-256 hashes are stored. Plaintext `ok-` tokens are shown once at
creation.
...
`https://api.onekey.dev`) and `ANTHROPIC_API_KEY` to your `ok-` keychain
key. Claude Code sends `x-api-key`, which the gateway accepts.

### Can I use the official OpenAI Python SDK?

Yes. Set `base_url` to your gateway `/v1` prefix and `api_key` to your `ok-`
key.
