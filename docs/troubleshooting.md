...
```sh
export DATABASE_URL="sqlite:////tmp/onekey.db"
```
...
- `https://www.onekey.dev` (hosted dashboard)
...
public gateway URL such as `https://api.onekey.dev`, not
`http://localhost:8000`.
...
- Use the **Onekey** `ok-` key, not the Supabase JWT
...
- **Local vs production:** Keys are stored per gateway database. A key minted on
  `localhost:8000` will not work on `api.onekey.dev`. On the production
...
Effort tiers are pseudo-models. Use `onekey-high` for the **best** free
models, `onekey-low` for the **fastest**. Customize the cascade in **Models**.
...
2. Search [GitHub Issues](https://github.com/Sharann-del/API_Keychain/issues)
3. Open a [bug report](https://github.com/Sharann-del/API_Keychain/issues/new?template=bug_report.yml) with logs (redact secrets)
