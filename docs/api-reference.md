...
| `/v1/chat/completions`, `/v1/responses`, `/v1/models` | `Authorization: Bearer ok-<onekey-key>` |
| `/v1/messages`, `/v1/messages/count_tokens` | `Authorization: Bearer ok-...` **or** `x-api-key: ok-...` |
...
- `onekey-low`, `onekey-medium`, `onekey-high`
...
**Headers:** `Authorization: Bearer ok-...` or `x-api-key: ok-...`
...
| `onekey-low` / `onekey-medium` / `onekey-high` | direct tier | Fast / balanced / best |
...
| --- | --- | --- |
| `GET` | `/users/{user_id}/onekey-keys` | List Onekey keys (masked) |
| `POST` | `/users/{user_id}/onekey-keys` | Create new `ok-` key |
| `POST` | `/users/{user_id}/regenerate-key` | Rotate primary key |
| `PUT` | `/onekey-keys/{key_id}` | Update label or rate limit |
| `DELETE` | `/onekey-keys/{key_id}` | Revoke a key |
