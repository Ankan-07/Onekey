...
| `DATABASE_URL` | No | SQLAlchemy URL (default: SQLite file `onekey.db` in project root) |
...
if you only use Onekey key auth.
...
```sh
export CORS_ORIGINS="http://localhost:3000,https://www.onekey.dev,https://onekey.dev,https://custom.example"
```
...
| CORS allowed origins | `main.py` (`CORS_ORIGINS`) | `http://localhost:3000`, `https://www.onekey.dev`, `https://onekey.dev` |
...
| Onekey key rate limit | `PUT /onekey-keys/{key_id}` | Requests per minute per `ok-` key |
