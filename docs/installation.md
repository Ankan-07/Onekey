...
Onekey consists of two deployable components:
...
```sh
export DATABASE_URL="sqlite:////absolute/path/to/onekey.db"
```
...
By default the database file is `onekey.db` in the project root (resolved
...
| Component | URL |
| --- | --- |
| Dashboard | `https://www.onekey.dev` |
| Gateway | `https://api.onekey.dev` |
...
| Setting | Value |
| --- | --- |
| Site URL | `https://www.onekey.dev` |
| Redirect URLs | `https://www.onekey.dev/auth/callback` |

Add `https://onekey.dev/**` if the apex domain also serves the dashboard.
...
NEXT_PUBLIC_API_BASE_URL=https://api.onekey.dev
```
...
5. CORS is controlled by `CORS_ORIGINS` (comma-separated). Default when unset:
   ...
   `http://localhost:3000,https://www.onekey.dev,https://onekey.dev`

   Append additional origins (e.g. a custom domain) via the `CORS_ORIGINS`
   environment variable on the gateway host.

6. **Custom API domain** (`api.onekey.dev`): Render → your web service →
   **Settings** → **Custom Domains** → add `api.onekey.dev`. Create a
   **CNAME** at your DNS host: `api` → the hostname Render shows (often
   `onekey.onrender.com`). SSL is issued automatically. Then set
   `NEXT_PUBLIC_API_BASE_URL=https://api.onekey.dev` on Vercel and redeploy
   the dashboard so code snippets and API calls use the new URL.

### Database

**Render's filesystem is ephemeral.** The default `onekey.db` SQLite file is
