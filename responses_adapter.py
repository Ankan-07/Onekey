...
This module translates requests and SSE streams between OpenAI's Responses API
(used by Codex CLI v0.136+) and standard Chat Completions payloads.
It must NEVER directly invoke network clients or touch the database.
"""
...
def resolve_responses_effort(body: Dict[str, Any]) -> str:
    """Extract effort level ('low', 'medium', 'high') from body.

    Checks 'onekey-<tier>' model prefix, then reasoning.effort, defaulting to 'medium'.
    """
...
def responses_to_openai_body(body: Dict[str, Any]) -> Dict[str, Any]:
    """Translate a Responses API request body to OpenAI Chat Completions body format."""
...
def openai_to_responses_response(
    ...
    """Translate an OpenAI Chat Completions response object into a Responses API response envelope."""
...
async def convert_openai_stream_to_responses(
    ...
    """Translate Chat Completions SSE (plus Onekey routing events) to Responses SSE."""
    response_id = _new_id("resp")
    created_at = int(time.time())
