/**
 * lib/keystore.ts — localStorage cache for plaintext ok- keys.
 *
 * The backend returns an `ok-...` Onekey key in plaintext exactly once (on
 * create / regenerate). We persist the primary key locally so the dashboard
 * can reveal it to the user on demand. Keys are scoped to the gateway URL
 * they were minted against so a key from localhost is never sent to production.
 *
 * REBRAND: All localStorage keys use `ok_key_*` prefix (was `ak_key_*`).
 */

function storageKey(userId: string): string {
  return `ok_key_${userId}`;
}

function metaStorageKey(userId: string): string {
  return `ok_key_meta_${userId}`;
}

function playgroundKeyStorageKey(userId: string): string {
  return `ok_playground_key_${userId}`;
}
