/**
 * lib/catalog.ts — Static mirror of registry.py's provider + tier data.
 *
 * CRITICAL: This file must stay in sync with registry.py manually.
 * Drift here is the most common latent bug: the frontend shows models
 * the backend doesn't know about, or vice versa.
 *
 * Last synced: 2026-08-06 against registry.py MODEL_TIERS.
 */
...
export interface ProviderMeta {
  ...
  iconUrl?: string;
  /** Free-tier models this provider serves, available through Onekey. */
  freeModels: string[];
...
// ---------------------------------------------------------------------------
// PROVIDERS — mirrors registry.py's PROVIDERS dict (12 entries).
// Keep slug, baseUrl, and freeModels in sync with registry.py.
// ---------------------------------------------------------------------------
export const PROVIDERS: ProviderMeta[] = [
  ...
  },
  {
    ...
    domain: "nvidia.com",
    // No local icon — favicon from nvidia.com is fine
    freeModels: [
...
// ---------------------------------------------------------------------------
// TIERS — mirrors registry.py's MODEL_TIERS exactly.
// SYNC CHECK: compare against registry.py before every release.
// ---------------------------------------------------------------------------
...
export const TIERS: TierMeta[] = [
  {
    ...
    // Mirrors registry.py MODEL_TIERS["low"]
    models: [
      "gemini-2.0-flash",
  ...
  },
  {
    ...
    // Mirrors registry.py MODEL_TIERS["medium"]
    models: [
      "gemini-2.0-flash",
  ...
  },
  {
    ...
    label: "Best",
    blurb:
      "Strongest free models — DeepSeek v4, Gemini 2.5 Pro, and Nemotron Ultra for hard problems.",
    // Mirrors registry.py MODEL_TIERS["high"]
    models: [
      "gemini-2.5-pro",
...
export const TOTAL_PROVIDERS = PROVIDERS.length; // must be 12

/** All provider slugs from the static catalog. */
export const PROVIDER_SLUGS = PROVIDERS.map((p) => p.slug);

/** Real company logo via DuckDuckGo's favicon service (no API key needed). */
